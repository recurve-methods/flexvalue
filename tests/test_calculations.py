...
def test_user_inputs_from_example_metered(snapshot):
    ...
    user_inputs = get_example_user_inputs_metered()
    df_output_table, elec_benefits, gas_benefits = flexvalue_run.get_results(
        user_inputs
    )
    snapshot.assert_match(df_output_table, "df_output_table")
...
def test_user_inputs_basic(user_inputs, metered_load_shape, snapshot, flexvalue_run):
    df_output_table, _, _ = flexvalue_run.get_results(user_inputs)
    snapshot.assert_match(df_output_table, "df_output_table")


def test_user_inputs_full(user_inputs, metered_load_shape, snapshot, flexvalue_run):
    df_output_table, elec_ben, gas_ben = flexvalue_run.get_results(user_inputs)
    snapshot.assert_match(df_output_table, "df_output_table")
...
def test_user_inputs_single_row(
    ...
    df_output_table, elec_ben, gas_ben = flexvalue_run.get_results(user_inputs[-1:])
    snapshot.assert_match(df_output_table, "df_output_table")
